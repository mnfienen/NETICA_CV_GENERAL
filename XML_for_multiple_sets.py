...
for i in np.arange(numfolds):
    ofp = open(infile[:-4] + '_set%d_.xml' %(i+1),'w')
    for line in indat:
        if '<baseCAS>' in line:
            ofp.write(re.sub(baseCAS,baseCAS[:-4] + '_set%d_.cas' %(i+1),line.strip()) + '\n')
        elif '<name>' in line:
            ofp.write(re.sub(scenarioname,scenarioname + '_set%d' %(i+1),line.strip()) + '\n')
        else:
